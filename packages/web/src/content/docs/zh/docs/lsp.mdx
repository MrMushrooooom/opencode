...
opencode 与您的语言服务器协议 (LSP) 集成，帮助 LLM 与您的代码库交互。它使用诊断来为 LLM 提供反馈。并使用 _转到定义_ 和 _查找引用_ 来帮助导航您的代码库。
...
| gopls      | .go                                                  | 可用的 `go` 命令                    |
| ruby-lsp   | .rb, .rake, .gemspec, .ru                            | 可用的 `ruby` 和 `gem` 命令        |
...
您可以通过 opencode 配置中的 `lsp` 部分来自定义 LSP 服务器。
