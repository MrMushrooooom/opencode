...
opencode 在文件被写入或编辑后自动使用特定语言的格式化器进行格式化。这确保生成的代码遵循您项目的代码风格。

---

## 内置

opencode 为流行的语言和框架提供了几个内置格式化器。以下是格式化器列表、支持的文件扩展名以及它需要的命令或配置选项。

| 格式化器      | 扩展名                                                                                     | 要求                          |
| -------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| gofmt          | .go                                                                                                      | `gofmt` 命令可用             |
| mix            | .ex, .exs, .eex, .heex, .leex, .neex, .sface                                                             | `mix` 命令可用               |
| prettier       | .js, .jsx, .ts, .tsx, .html, .css, .md, .json, .yaml, 和 [更多](https://prettier.io/docs/en/index.html) | `package.json` 中有 `prettier` 依赖 |
...
所以如果您的项目在 `package.json` 中有 `prettier`，opencode 会自动使用它。
...
此过程在后台进行，确保您的代码风格得到维护，无需任何手动步骤。

---

## 配置

您可以通过 opencode 配置中的 `formatter` 部分自定义格式化器。
...
您可以通过指定命令、环境变量和文件扩展名来覆盖内置格式化器或添加新的格式化器：
