...
export const FileChanges: React.FC<FileChangesProps> = ({ fileChanges }) => {
  if (fileChanges.length === 0) {
    return <div style={{ color: "#888888", fontSize: "12px" }}>No file changes</div>
  }
        ...
        renderItem={(change) => (
          <List.Item style={{ padding: "4px 0" }}>
            <Space>
              ...
              <Text style={{ color: "#cccccc", fontSize: "12px" }}>{change.filePath}</Text>
              <Tag color={getChangeColor(change.type)}>{change.type}</Tag>
