...
import type { FrontendAppState, Session, Provider, Model, Agent, Message, FrontendPart, Permission } from "../types"
...
export const useAppStore = create<FrontendAppState>((set, get) => ({
  ...
  removePermission: (permissionId: string) =>
    set((state) => {
      ...
      return {
        ...
        currentPermission:
          state.currentPermission?.id === permissionId ? updatedPermissions[0] || null : state.currentPermission,
      }
    }),
  ...
  updateMessage: (messageId: string, updates: Partial<Message>) =>
    set((state) => ({
      messages: state.messages.map((message) => (message.info.id === messageId ? { ...message, ...updates } : message)),
    })),

  updateMessagePart: (messageId: string, part: FrontendPart) =>
    set((state) => ({
